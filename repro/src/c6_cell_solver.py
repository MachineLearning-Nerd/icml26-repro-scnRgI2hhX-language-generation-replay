...
CELLS = (
    Cell("x<=-3", None, -3), Cell("x=-2", -2, -2), Cell("x=-1", -1, -1),
    Cell("x=0", 0, 0), Cell("x=1", 1, 1), Cell("x=2", 2, 2), Cell("x>=3", 3, None),
)
...
def certify(mutated: bool = False) -> dict:
    supports = {
        "h1-": support_vector("minus", 1), "h2-": support_vector("minus", 2),
        "h1+": support_vector("plus", -1), "h2+": support_vector("plus", None if mutated else -2),
    }
    complete_partition = tuple((cell.lower, cell.upper) for cell in CELLS) == (
        (None, -3), (-2, -2), (-1, -1), (0, 0), (1, 1), (2, 2), (3, None),
    )
    branches = []
    for first in supports:
        ...
        expected = tuple(atom_truth(cell, ">=" if minus_first else "<=", 0) for cell in CELLS)
        no_subset = all(any(member and not shared for member, shared in zip(vector, intersection)) for vector in supports.values())
        branches.append({
            "first_output": first,
